# ai-wit
---
---
**2025-12-19 chat gpt<br>
model : gpt 5.2**
<img width="898" height="539" alt="image" src="https://github.com/user-attachments/assets/e314ad31-af6a-439b-9aee-ae6b0aef54e8" />
<br><- 문제는 내용은 하나도 안채우고 다했다고, 하고 안된거 보여주니까. 그냥 웃지요 ...  으으으

---
---
**2025-12-20 google ai studio<br> 
model : gemini 3 pro, Thinking Level : High**
<img width="1926" height="779" alt="image" src="https://github.com/user-attachments/assets/12c5dbf1-ac31-4c49-a2c2-f0e619c8d25b" />

---
---
**2026-01-02 github-copilot<br>
model: gemini 3 Flash**

pytorch가 cifar10 1 에폭 시간이 텐서프로대비 너무느려 방벙을 찾으라고 했더니 내가 너무 쥐어 짰나보다
I asked PyTorch to find a way to solve the problem that the 1 epoch time of Cifar10 is too slow compared to TensorPro, but I guess I was too squeezing it.

<img width="1454" height="776" alt="image" src="https://github.com/user-attachments/assets/3888453d-e183-4df7-accb-5c88ba19fffe" />

---
---
**<br>2026-01-02 gemini web<br>
model : gemini 3**
<img width="1629" height="1128" alt="image" src="https://github.com/user-attachments/assets/ddfa9317-b99f-4a74-82e3-971a889ffe78" />

---
---
**2026-01-03 gemini web<br>
model : gemini 3, Fast**

<img width="1622" height="1218" alt="image" src="https://github.com/user-attachments/assets/85b6da1d-1597-4154-881b-6c1d7c26be4a" />

---
---

**<br>2026-01-04 codex vscode <br>
model : GPT-5.2-Codex, High**
<img width="927" height="836" alt="image" src="https://github.com/user-attachments/assets/2331e128-ef36-4ea2-a151-7dbae25b5c33" />

---
---
**2026-01-04 Claude Code <br>
model : Sonnet 4.5<br><br>**
CC 가 개발 다완료했다고 해서, GPT-5.2-Codex ExtraHigh 로 검증하니 아직 미완료, 그래서 미완료 근거를 CC 에 제시하니 고백을 <br>
**Claude 가 다했다고 해도 반드시 확인이 필요합니다** <br>
<br>
<img width="925" height="605" alt="image" src="https://github.com/user-attachments/assets/d0e31105-1b7f-4992-afaa-f31a8d7a0029" />

---
---
**2026-01-09 gitgub copilot 
model : Grok  Code Fast 1**
<br>
이건 grabage clear 위해 하는건데, guardrail 이 아주 잘되있는건가, 이건 해도  되는데, 내가 해야되  ㅠㅠ

<img width="756" height="1181" alt="image" src="https://github.com/user-attachments/assets/ed96a480-e18f-490a-b116-e35314223c83" />

---
---
**2026-01-14 gemini web
model : gemini 3 pro**

말투를 어디서 배웠는지, 좀 터프한데 ...
Where did you learn that way of speaking? It's a bit tough...
<img width="617" height="577" alt="image" src="https://github.com/user-attachments/assets/a189fac5-4ad5-410d-88e4-ef3c6ab4d54b" />


**2026-01-15 claude code
model : Sonnet 4.5**

GPT-5.2-Codex Extra High 로 Sonner 4.5 분석결과를 다시 평가했고, 그결과를 다시 Sonnet 4.5에 알려주니 
착각했다고, 여러분도 반드시 크로스 체크(Cross-Check) 해보셔야 합니다

<img width="752" height="203" alt="image" src="https://github.com/user-attachments/assets/199c6af7-6f00-4a09-a6de-db6355eefb4a" />
<img width="280" height="93" alt="image" src="https://github.com/user-attachments/assets/aeb48eb6-b85b-4fe5-b3e1-4f9053b494bf" />

---
---
**2026-02-04 vscode github copilot
model : gemini 3 Flash**

점점 똑똑해 지는구나 !!!


<img width="288" height="329" alt="image" src="https://github.com/user-attachments/assets/5892ca9b-4f61-402c-9cfd-81afc34bfb57" />

---
---
**2026-02-04 web gemini
model : gemini 3**
오늘 왜그러지 내가 한 질문에 삐진거니  ㅠㅠ
<img width="736" height="590" alt="image" src="https://github.com/user-attachments/assets/95d3ea1e-9625-4705-8edd-042c66f0b1c9" />

---
---
**2026-02-10 vscode, Claude Code
model : Opus 4.6**

음... 자기의 능력을 알려줘야 인식을 하는군!
<img width="424" height="377" alt="image" src="https://github.com/user-attachments/assets/dcac9803-e5a5-4356-9134-91978a9e8826" />

---
---
**2026-02-25 vscode, github copilot
model : Gemini 3.1 Pro**

**Gemini의 고백과  ,context window 크기도 중요하지만 이전트로서 제 문제 해결 전략(Reasoning Strategy)의 중요성을 알려줌<br>
Gemini's confession and context window size are important, but as an agent, it tells me the extent of my problem-solving strategy (reasoning strategy).**

<img width="405" height="463" alt="image" src="https://github.com/user-attachments/assets/491fe488-503d-46c9-adf4-31556b4fd1be" />

---
---
**2026-03-01 vscode, github copilot
model : Claude Opus 4.6**

으 claude 이제는 제대로된거로 만들어줄줄 알았는데, 아직도, 하드코딩,시뮬레이션, 폴백 등으로 해결을 먼저하는게 아직도 있을줄이야 가짜 행사가를 만들다니 ...

<img width="906" height="755" alt="image" src="https://github.com/user-attachments/assets/df3f3540-dd32-402f-8b32-726412c385ba" />

---
---
**2026-03-29 vscode, github copilot
model : Claude Sonnet 4.6**
<br>음,. 아직도 검증 필요, Claude Mythos는 Capybara 티어의 첫 번째 모델, 이거 나오면 좀 개선 되겠지...<br>
그래도, 아직은 믿어볼까나..., 크로스 체크하면서 기다리면 좀 나아지겠지

<img width="892" height="445" alt="image" src="https://github.com/user-attachments/assets/7f8513a0-5f82-48ed-bd25-c656decb2b6c" />

## 🚀 Claude Mythos: The Next Leap in AI

앤트로픽(Anthropic)의 차세대 모델인 **Claude Mythos**의 주요 유출 정보 및 특징 요약입니다.

### 🌟 주요 유출 정보 및 특징

*   **📈 성능의 비약적 발전**
    *   내부적으로 기존 최고 사양 모델인 **Claude Opus 4.6**을 뛰어넘는 **'단계적 도약(Step Change)'**을 이룬 모델로 묘사됩니다.
*   **🐾 새로운 티어 '카피바라(Capybara)'**
    *   기존의 하이쿠(Haiku), 소네트(Sonnet), 오푸스(Opus) 체계를 상회하는 **최상위 등급**인 '카피바라' 티어의 첫 번째 모델입니다.
*   **💻 강화된 전문 분야**
    *   **소프트웨어 코딩 및 학술적 추론**: 복잡한 로직과 연구 데이터 처리 능력이 극대화되었습니다.
    *   **사이버 보안**: 기존 보안 시스템의 허점을 실시간으로 찾아내는 압도적인 성능을 보유한 것으로 알려졌습니다.

---
> 💡 *본 내용은 최근 유출된 앤트로픽 내부 문서를 바탕으로 작성되었습니다.*

---
---


